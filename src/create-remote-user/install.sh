...
if [ "$ADDTOSUDO" = "true" ]; then
  echo "Configuring sudo access for user $_REMOTE_USER"
  
  if [ "$RUNSUDOWITHOUTPASSWORD" = "true" ]; then
    echo "Configuring passwordless sudo for user $_REMOTE_USER"
    configure_passwordless_sudo "$_REMOTE_USER"
  else
    echo "Configuring password-required sudo for user $_REMOTE_USER"
    configure_password_sudo "$_REMOTE_USER"
  fi
fi
